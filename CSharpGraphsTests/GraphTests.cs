...
namespace CSharpGraphsTests
{
    ...
    public class GraphTests
    {
        ...
        {
            var graph = Graph<int>.Create(1, 2, 3);
            bool con = graph.TryConnect(1, 2, true);
            if (graph.EdgeCount != 1 || !con) Assert.Fail();
            con = graph.TryConnect(2, 1, true);
            if (graph.EdgeCount != 1 || !con) Assert.Fail();
            con = graph.TryConnect(2, 3, false);
            if (graph.EdgeCount != 2 || !con) Assert.Fail();
            con = graph.TryConnect(2, 3, true);
            if (graph.EdgeCount != 2 || !con) Assert.Fail();
            con = graph.TryConnect(3, 2, true);
            if (graph.EdgeCount != 2 || !con) Assert.Fail();
            Assert.Pass();
        }
        [Test]
        public void TryConnect2()
        {
            ...
            bool con = graph.TryConnect(1, 2, true);
            if (graph.EdgeCount != 1 || !con) Assert.Fail();
            con = graph.TryConnect(1, 5, false);
            if (graph.EdgeCount != 2 || !con) Assert.Fail();
            con = graph.TryConnect(2, 6, true);
            if (graph.EdgeCount != 3 || !con) Assert.Fail();
            con = graph.TryConnect(2, 4, false);
            if (graph.EdgeCount != 4 || !con) Assert.Fail();
            con = graph.TryConnect(5, 3, true);
            if (graph.EdgeCount != 5 || !con) Assert.Fail();
            con = graph.TryConnect(4, 3, false);
            var edges = graph.Edges()!;
